...
import { initialPassengerRates, initialShipperRates, initialEquipmentRates } from "./price/page";
...
export function ManualDispatchForm() {
  ...
  const [passengerRates, setPassengerRates] = useState(initialPassengerRates);
  const [shipperRates, setShipperRates] = useState(initialShipperRates);
  const [equipmentRates, setEquipmentRates] = useState(initialEquipmentRates);
  ...
  useEffect(() => {
    if (distance > 0 && vehicleType) {
        let ratePerKm: number | undefined;
        if (bookingType === 'passenger' && passengerRates[vehicleType]) {
          ...
          if (shipperRates[vehicleType]) {
            ...
            // Assuming per-km for equipment for now
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                  ...
                  <div className="space-y-2">
                      ...
                      <Input id="fare" type="number" placeholder="Enter fare amount" value={fare > 0 ? fare.toFixed(2) : ""} onChange={(e) => setFare(parseFloat(e.target.value))} disabled={isConfirming} />
                  </div>
              </div>
