...
import type { User, Trip, VehicleType } from "@/lib/types";
...
const freightVehicleTypes: VehicleType[] = [
...
export function ShipperPortalClient({ allUsers }: ShipperPortalClientProps) {
  ...
  const [pickupAddress, setPickupAddress] = useState("Dammam Port");
  const [dropoffAddress, setDropoffAddress] = useState("Riyadh Logistics Hub");
  ...
  const handleConfirmBooking = () => {
    ...
    const vehicleForType = vehicles.find(v => v.type === selectedVehicle);
    ...
    const newTrip: Trip = {
      ...
      type: "SHIPMENT",
      status: "PENDING",
      pickupLocation: { address: pickupAddress, lat: 26.4333, lng: 50.1167 },
      dropoffLocation: { address: dropoffAddress, lat: 24.7136, lng: 46.6753 },
      scheduledAt: scheduledAt.toISOString(),
      distance: 400, // Estimated
      fare: 450.00,
      estimatedDuration: 240, // Estimated in minutes
    ...
    toast({ title: "Shipment Confirmed!", description: `Your booking for a ${selectedVehicle.replace(/_/g, ' ')} has been confirmed and will appear on the dashboard.` });
    ...
    <div>
        ...
        <CardContent className="space-y-6">
            ...
            <div>
                ...
                <Select value={selectedVehicle} onValueChange={(value: VehicleType) => setSelectedVehicle(value)}>
